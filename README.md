...
苏格拉底式创业计划书打磨技能。不讲课，不判断好坏，只发问。通过 6 个阶段、136 道问题，帮创业者把想法打磨成完整的创业计划。最终输出两份文档：过程记录 + 10章创业计划书。
...
│   ├── Stage-4.md
│   └── Stage-5.md
├── validate-skill.sh     # 结构自检脚本（20 项9维合规检查）
├── test-prompts.json     # 测试用例（12 条，含 happy-path + 边界场景）
├── skill-banner.png      # 技能包展示图片
├── README.md             # 本文件
└── docs/                 # 设计文档与实施计划
```
...
> "grill me" / "创业" / "商业计划书" / "创业计划书" / "打磨" / "过一遍" / "梳理想法"

### 用户指令

| 用户可以说 | 效果 |
|-----------|------|
| "跳过Y阶段"（如"跳过第3阶段"） | 跳过 Y 阶段，标记为【已跳过】 |
| "重新回答X阶段"（如"重新回答第2阶段"） | 重新回答 X 阶段，清空答案从头追问 |
| "给我看目前的计划书草稿" | 查看当前计划书草稿 |
| "这部分专业性较强" | 触发专业建议提示（财务/融资等） |
...
2. **提问后给出上下文提示**：基于用户之前的回答，提示帮用户关联已有信息、找到思考切入角度（Q0.1 和每个新阶段前 1-2 题不给提示）
3. 用户回答后，按 REFERENCE.md 中的 4 级追问力度处理（温和探询 → 聚焦追问 → 挑战检验 → 红色警报）
4. 答案直接记入计划书草稿，阶段结束生成结构化输出
5. 所有阶段完成后，输出两份文档：过程记录 + 10章创业计划书
...
1. **过程记录** (`grill记录_[项目名称].md`)：完整对话记录，包含所有提问、回答、提示、旁白注释、阶段摘要、矛盾和红色警报
2. **创业计划书** (`创业计划书_[项目名称].md`)：10章结构化商业计划书（7000-11000字），覆盖执行摘要、价值主张、市场分析、商业模式、验证计划、增长策略、团队组织、财务预测、融资计划、行动清单
